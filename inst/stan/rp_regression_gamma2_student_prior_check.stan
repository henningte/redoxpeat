data{
  ...
  vector[k] mu_betas; // mean of Student-t prior for slopes
  vector[k] sigma_betas; // sd of Student-t prior for slopes
  real<lower = 0> nu_betas; // df of Student-t prior for slopes
...
generated quantities{
  ...
  real beta_0 = normal_rng(mu_beta_0, sigma_beta_0);
  ...
  for (i in 1:k) {
    betas[i] = student_t_rng(nu_betas, mu_betas[i], sigma_betas[i]);
  }
  for(i in 1:N_j) {
